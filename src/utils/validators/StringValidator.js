...
class StringValidator extends Validator {
  ...
  matches(pattern, message) {
    if (!(pattern instanceof RegExp)) {
      throw new Error(
        `[${this.value}] ${
          message || StringValidator.#ERROR_MESSAGES.INVALID_REGEXP
        }`
      )
    }
    ...
    if (!pattern.test(this.value)) {
      throw new Error(
        `[${this.value}] ${
          message || StringValidator.#ERROR_MESSAGES.INVALID_PATTERN
        }`
      )
    }
  ...
  notEmpty(message) {
    if (this.value === '') {
      throw new Error(
        `[${this.value}] ${
          message || StringValidator.#ERROR_MESSAGES.EMPTY_STRING
        }`
      )
    }
